...
@section('page-header')
<x-page-header 
    :pageTitle="__('menu.Users')" 
    :currentPageTitle="__('menu.Users')"
    :haveSearch="true" 
    :linkCache="false" 
    :haveCalendarSearch="false" 
    :pagesBreadcrumb="[]"
    :routePageCreate="route('user.create')" 
    :routeNamePageCreate="__('partials.Create')" 
    dataMethodCreate="user.create" 
/>
@endsection
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered">
                                    <thead>
                                        <tr>
                                            <th>{{__('user::language.field.name')}}</th>
                                            <th>{{__('user::language.field.email')}}</th>
                                            <th>{{ __('user::language.field.registerDate') }}</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{$row->name}}</td>
                                            <td>{{$row->email}}</td>
                                            <td>{{ $row->created_at->format(config('helpers.dateTimeFormat12')) }}</td>
                                            <td>
                                                <x-edit :href="route('user.edit',[$row->id])" data-method="user.edit" />
                                                <x-delete :href="route('user.destroy',[$row->id])"
                                                    data-method="user.destroy" />
                                                <x-toggle :isActive="$row->is_active"
                                                    :url="route('user.activated',[$row->id])"
                                                    dataMethod="user.activated" />
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                ...
                <form action="" method="GET">
                    <div class="modal-body">
                        <fieldset class="form-group control-group floating-label-form-group control-group">
                            <label for="name">{{__('user::language.field.name')}}</label>
                            <input type="text" name="name" value="{{request()->name}}" class="form-control" id="name"
                                placeholder="{{__('user::language.field.pleaseEnterName')}}">
                        </fieldset>
                        <br>
                        <fieldset class="form-group control-group floating-label-form-group control-group">
                            <label for="email">{{__('user::language.field.email')}}</label>
                            <input type="email" name="email" value="{{request()->email}}" class="form-control"
                                id="email" placeholder="{{__('user::language.field.pleaseEnterEmail')}}">
                        </fieldset>
                    </div>
