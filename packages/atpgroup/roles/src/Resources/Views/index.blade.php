...
<x-page-header 
    ...
    :currentPageTitle="__('menu.Roles')"
    :haveSearch="true" 
    :linkCache="false" 
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered">
                                    <thead>
                                        <tr>
                                            <th>{{ __('role::language.field.name') }}</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($row->id != 1)
                                                    <x-edit :href="route('role.edit',[$row->id])" data-method="role.edit" />
                                                @endif
                                                @if (!in_array($row->id, [1]))
                                                <x-delete :href="route('role.destroy',[$row->id])"
                                                    data-method="role.destroy" />
                                                @endif
                                            </td>
                                        </tr>
                                        @endforeach
                ...
                <form action="" method="GET">
                    <div class="modal-body">
                        <fieldset class="form-group control-group floating-label-form-group control-group">
                            <label for="name">{{__('role::language.field.name')}}</label>
                            <input type="text" name="name" value="{{request()->name}}" class="form-control" id="name"
                                placeholder="{{__('role::language.field.pleaseEnterName')}}">
                        </fieldset>
                    </div>
